...
template<class T>
constexpr const T& aq_max(const T& left, const T& right) noexcept {
...
public:
    ...
    unsigned was_size() const noexcept {
        ...
        return atomic_queue::details::aq_max(static_cast<int>(head_.load(X) - tail_.load(X)), 0);
    }
...
public:
    ...
    AtomicQueueB(unsigned size)
        : size_(atomic_queue::details::aq_max(details::round_up_to_power_of_2(size), 1u << (SHUFFLE_BITS * 2)))
...
public:
    ...
    AtomicQueueB2(unsigned size)
        : size_(aq_max(details::round_up_to_power_of_2(size), 1u << (SHUFFLE_BITS * 2)))
